...
export default function Anotacoes() {
  ...
  const notasFiltradas = notas.filter((n) => n.titulo.toLowerCase().includes(filtro.toLowerCase()) || n.corpo.toLowerCase().includes(filtro.toLowerCase()))
  ...
  return (
    <div className="an-container">
      ...
      <div className="an-header-acoes">
        <label htmlFor="an-search" className="sr-only">
          Pesquisar notas
        </label>
        <input id="an-search" name="search" type="text" className="an-search" placeholder="Pesquisar em notas..." value={filtro} onChange={(e) => setFiltro(e.target.value)} />
      </div>
      ...
      <div className="an-grid">
        ...
        {notasFiltradas.map((nota) => {
          ...
          return (
            <div key={nota.id} className="an-card" onClick={() => abrirModal(nota)}>
              <div className="an-card-header">
                <h3 className="an-card-titulo">{nota.titulo}</h3>
                <span className={`an-status-badge an-status-${nota.status.replace(/\s+/g, '-')}`}>{nota.status}</span>
              </div>
              ...
              {totalTasks > 0 && (
                <div className="an-card-checklist">
                  {nota.tasks.slice(0, 3).map((task, idx) => (
                    <div key={idx} className={`an-mini-task ${task.completed ? 'completed' : ''}`}>
      ...
      <Modal aberto={modalAberto} onFechar={fecharModal} titulo="Editor de Nota" largura="600px">
        {editando && (
          <>
            <div className="an-input-group">
              <label htmlFor="an-titulo" className="an-label">
                Título
              </label>
              <input id="an-titulo" name="titulo" type="text" className="an-input-titulo" placeholder="Título da nota..." value={editando.titulo} onChange={(e) => setEditando({ ...editando, titulo: e.target.value })} autoFocus />
            </div>

            <div className="an-input-group">
              <label htmlFor="an-status" className="an-label">
                Status
              </label>
              <select
                id="an-status"
            ...
            <div className="an-input-group">
              <label htmlFor="an-corpo" className="an-label">
                Conteúdo
              </label>
              <textarea id="an-corpo" name="corpo" className="an-textarea" placeholder="Escreva algo..." value={editando.corpo} onChange={(e) => setEditando({ ...editando, corpo: e.target.value })} />
            </div>
              ...
              {editando.tasks.map((task, idx) => (
                <div key={idx} className="an-task-row">
                  <label className="an-checkbox-wrapper" htmlFor={`task-check-${idx}`}>
                    ...
                    <input type="checkbox" id={`task-check-${idx}`} name={`task-check-${idx}`} checked={task.completed} onChange={() => toggleTask(idx)} />
                    <span className="an-checkmark" />
                  </label>

                  <div style={{ flex: 1 }}>
                    <label htmlFor={`task-text-${idx}`} className="sr-only">
                      Descrição da tarefa {idx + 1}
                    </label>
            ...
            <footer className="an-modal-footer">
              ...
              <div style={{ display: 'flex', gap: '10px' }}>
                <button className="an-btn-cancelar" onClick={fecharModal}>
                  Cancelar
                </button>
