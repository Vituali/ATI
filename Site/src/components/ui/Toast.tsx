 ...
 */
function ToastItem({ item, aoRemover }: { item: Notification; aoRemover: (id: string, resultado?: boolean) => void }) {
  ...
  return (
    <div className={`toast-item toast-${type} ${saindo ? 'exiting' : ''}`} id={`toast-${id}`} role="alert">
      ...
      <div className="toast-content">
        ...
        {type === 'confirm' && (
          <div className="toast-buttons">
            <button className="toast-btn toast-btn-confirm" onClick={() => fechar(true)}>
              Confirmar
            </button>
            <button className="toast-btn toast-btn-cancel" onClick={() => fechar(false)}>
              Cancelar
            </button>
          </div>
        )}
