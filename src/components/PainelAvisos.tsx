...
export default function PainelAvisos({ user }: PainelAvisosProps) {
  ...
  async function carregarAvisos() {
    ...
    try {
      ...
      setErro("Erro ao carregar avisos: " + e.message);
    } finally {
      setLoading(false);
    }
  }
  ...
  async function handleSalvar() {
    ...
    try {
      if (editandoId) {
        ...
        mostrarFeedback("Aviso atualizado com sucesso!", "ok");
      } else {
        // Novo
        ...
        mostrarFeedback("Novo aviso criado!", "ok");
      }
  ...
  async function toggleAtivo(aviso: Aviso) {
    try {
      ...
      mostrarFeedback(aviso.ativo ? "Aviso desativado." : "Aviso ativado!", "ok");
      carregarAvisos();
    } catch (e: any) {
      mostrarFeedback("Erro ao alterar status: " + e.message, "erro");
    }
  }
  ...
  async function handleExcluir(id: string) {
    try {
      ...
      mostrarFeedback("Aviso excluído permanentemente.", "ok");
      setConfirmandoExcluir(null);
      carregarAvisos();
    } catch (e: any) {
      mostrarFeedback("Erro ao excluir: " + e.message, "erro");
    }
  }
  ...
  return (
    <div className="painel-avisos">
      ...
      <div className="admin-header" style={{ borderBottom: "none", marginBottom: "0" }}>
        <div>
          <h2 className="admin-titulo" style={{ fontSize: "1.3rem" }}>📢 Gerenciar Avisos</h2>
      ...
      {feedback && (
        <div className={`admin-toast ${feedback.tipo}`} style={{ top: "80px" }}>
      ...
      {/* Formulário Inline */}
      {mostrandoForm && (
        <div className="admin-card" style={{ marginBottom: "1.5rem", border: "1px solid var(--accent-blue-border)" }}>
          ...
          <div className="up-section" style={{ padding: "0" }}>
            ...
            <div className="up-grupo">
              ...
              <select
                ...
                value={form.tipo}
                onChange={(e) => setForm({ ...form, tipo: e.target.value as Aviso["tipo"] })}
              >
          ...
          <div className="admin-tabela-wrapper">
            <table className="admin-tabela">
              ...
              <tbody>
                {avisos.length === 0 && (
                  <tr>
                    <td colSpan={6} className="admin-vazio">Nenhum aviso encontrado</td>
                  </tr>
                )}
                {avisos.map((aviso) => (
                  <tr key={aviso.id} style={{ opacity: aviso.ativo ? 1 : 0.5 }}>
                    <td>
                      <div style={{ fontWeight: 700, color: "var(--text-white)" }}>{aviso.titulo}</div>
                      <div className="admin-email" style={{ whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis", maxWidth: "250px" }}>
                        {aviso.corpo}
                      </div>
                    </td>
                    <td>
                      <span className={`home-badge`} style={{ 
                    ...
                    <td>
                      <button 
                        ...
                        style={{ 
                          background: aviso.ativo ? "rgba(255, 82, 82, 0.1)" : "rgba(0, 200, 83, 0.1)",
                    ...
                    <td className="admin-username">@{aviso.criadoPor}</td>
                    <td className="admin-email">{new Date(aviso.timestamp).toLocaleDateString()}</td>
                    <td>
                      <div style={{ display: "flex", gap: "8px" }}>
                        {confirmandoExcluir === aviso.id ? (
                          <>
                            <button className="admin-btn-salvar" style={{ background: "#ff5252" }} onClick={() => handleExcluir(aviso.id)}>Sim</button>
                            <button className="admin-btn-salvar" style={{ background: "var(--bg-input)" }} onClick={() => setConfirmandoExcluir(null)}>Não</button>
                          </>
                        ) : (
                          <>
                            <button className="admin-btn-salvar" style={{ padding: "4px 8px", fontSize: "11px" }} onClick={() => abrirEdicao(aviso)}>Editar</button>
                            <button 
                              className="admin-btn-salvar" 
                              style={{ background: "transparent", color: "#ff5252", padding: "4px 8px", fontSize: "11px" }}
                              onClick={() => setConfirmandoExcluir(aviso.id)}
                            >
                              Excluir
                            </button>
                          </>
