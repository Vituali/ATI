...
export function getSgpClientData(): ClientData {
  ...
  if (fullName === 'Cliente') {
    ...
    const clientLink = allLinks.find((link) => {
      ...
      return (href.includes('/admin/cliente/cliente/') || href.includes('/admin/cliente/')) && !href.includes('/add/') && !href.includes('/list/') && !/^(alterar|excluir|histórico|historico|add|adicionar|limpar|voltar)$/i.test(link.textContent?.trim() || '')
    })
...
const SgpMenu: React.FC = () => {
  ...
  const handleToggleServer = async () => {
    ...
    const isGeneralPage = window.location.pathname === '/' || window.location.pathname === '/admin' || window.location.pathname === '/admin/'
  ...
  return (
    <div className="ati-sgp-container">
      {/* Botão Flutuante */}
      <button className={`ati-sgp-toggle ${isOpen ? 'active' : ''}`} onClick={() => setIsOpen(!isOpen)} title="Painel Auxiliar ATI SGP">
        {isOpen ? (
