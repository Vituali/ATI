...
export default function ModelosOS() {
  ...
  const carregarTudo = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const [snapModelos, snapCache35, snapCache53] = await Promise.all([
        get(ref(db, `modelos_os/${user.username}`)),
        get(ref(db, "sgp_cache/occurrenceTypes")),
        get(ref(db, "sgp_cache_53/occurrenceTypes")),
      ]);
        ...
        agrupados.map(({ cat, itens }) => (
          <div key={cat} className="modelos-os-grupo">
            ...
            <div className="modelos-os-lista">
              {itens.map((modelo) => {
                const tipo = (occurrenceTypes as any[]).find(
                  (o) => o.id35 === modelo.occurrenceTypeId || (modelo.occurrenceTypeId_53 && o.id53 === modelo.occurrenceTypeId_53),
                );
