...
async function loginWithUsername(username, password) {
  const attendantData = await getAtendenteByUsername(
...
async function createUserAccount(details) {
  ...
  // Aguarda o auth state ser confirmado ANTES de escrever no banco
...
function loadGoogleMapsScript() {
  ...
  script.src =
    "https://maps.googleapis.com/maps/api/js?key=AIzaSyB5wO0x-7NFmh6waMKzWzRew4ezfYOmYBI&libraries=places,routes&callback=initMap";
  script.async = true;
  script.defer = true;
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  const startApp = async (user, allAtendentes) => {
    const attendantKey = Object.keys(allAtendentes).find(
      (key) => allAtendentes[key].uid === user.uid,
    );
    if (!attendantKey) {
    ...
    window.dispatchEvent(
      new CustomEvent("atendenteAtualChanged", {
        detail: { newUser: attendantKey },
      }),
    );
    ...
    if (adminLinkContainer)
      adminLinkContainer.style.display =
        attendantData.role === "admin" ? "block" : "none";
  ...
  const showLoginScreen = () => {
    ...
    window.dispatchEvent(
      new CustomEvent("atendenteAtualChanged", { detail: { newUser: null } }),
    );
  };
  ...
  checkAuthState(async (user) => {
    if (user) {
      try {
        ...
        let attendantKey = Object.keys(allAtendentes).find(
          (key) => allAtendentes[key].uid === user.uid,
        );
        ...
        if (!attendantKey) {
          console.log(
            "Perfil não encontrado na primeira tentativa, aguardando 1s e tentando novamente...",
          );
          ...
          attendantKey = Object.keys(allAtendentes).find(
            (key) => allAtendentes[key].uid === user.uid,
          );
        }
        ...
        error.code === "auth/email-already-in-use"
          ...
          : error.code === "auth/weak-password"
            ...
            : error.message.includes(
                  "Este nome de usuário já pode estar em uso",
                )
  ...
  document.addEventListener("click", (e) => {
    ...
    if (copiableElement) {
      navigator.clipboard
        .writeText(copiableElement.textContent)
        .then(() =>
