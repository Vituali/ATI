...
export default function Relatorios() {
  ...
  useEffect(() => {
    ...
    const unsubscribePotencias = onValue(potenciasRef, (snapshot) => {
  ...
  const classificarSinal = useCallback((rxStr: string) => {
  ...
  const registrosFiltrados = useMemo(() => {
    const filtered = registros.filter((reg) => {
      ...
      const bateBusca =
        (reg.login || "").toLowerCase().includes(query) ||
        (reg.nome || "").toLowerCase().includes(query) ||
        (reg.olt || "").toLowerCase().includes(query) ||
        (reg.bairro || "").toLowerCase().includes(query) ||
        (reg.endereco || "").toLowerCase().includes(query) ||
        (reg.contrato || "").toLowerCase().includes(query) ||
        (reg.pon || "").toLowerCase().includes(query) ||
  ...
  return (
    <div className="relatorios-page">
      <div className="relatorios-header flex-header">
        <div>
          ...
          <p className="relatorios-subtitulo">
            Análise em tempo real dos sinais coletados do SGP
          </p>
        </div>
          ...
          <div className="rel-config-grid">
            <div className="rel-config-group">
              ...
              <input
                ...
                step="0.1"
                value={inputMaxExc}
                onChange={(e) => setInputMaxExc(e.target.value)}
                required
              />
            ...
            <div className="rel-config-group">
              ...
              <input
                ...
                step="0.1"
                value={inputMinExc}
                onChange={(e) => setInputMinExc(e.target.value)}
                required
              />
      ...
      <div className="rel-metricas-grid">
        <div className="rel-metrica-card clickable" onClick={() => { setFiltroSinal("todos"); setFiltroStatus("todos"); }}>
        ...
        <div className="rel-metrica-card excelente clickable" onClick={() => setFiltroSinal("excelente")}>
          ...
          <div className="rel-metrica-info">
            ...
            <span className="rel-metrica-label">Excelente ({thresholds.minExcelente} a {thresholds.maxExcelente})</span>
          </div>
        </div>
        ...
        <div className="rel-metrica-card atencao clickable" onClick={() => setFiltroSinal("atencao")}>
          ...
          <div className="rel-metrica-info">
            ...
            <span className="rel-metrica-label">Atenção (&lt; {thresholds.minExcelente} ou &gt; {thresholds.maxExcelente})</span>
          </div>
        </div>
      ...
      <div className="rel-filtros-bar">
        <input
          type="text"
          placeholder="Buscar por OLT, Login, Bairro, PON..."
          value={busca}
          onChange={(e) => setBusca(e.target.value)}
          className="rel-busca-input"
        />
        <select
          value={filtroSinal}
          onChange={(e: any) => setFiltroSinal(e.target.value)}
          className="rel-select-filtro"
        >
        ...
        <div className="rel-colunas-toggles">
          <label className="rel-toggle-label">
            <input
              type="checkbox"
              checked={exibirVlan}
              onChange={(e) => setExibirVlan(e.target.checked)}
            />
            VLAN
          </label>
          <label className="rel-toggle-label">
            <input
              type="checkbox"
              checked={exibirSinalTx}
              onChange={(e) => setExibirSinalTx(e.target.checked)}
            />
            Sinal TX
          </label>
          <label className="rel-toggle-label">
            <input
              type="checkbox"
              checked={exibirRxOlt}
              onChange={(e) => setExibirRxOlt(e.target.checked)}
            />
            RX OLT
          </label>
          <label className="rel-toggle-label">
            <input
              type="checkbox"
              checked={exibirDataColeta}
              onChange={(e) => setExibirDataColeta(e.target.checked)}
            />
            Data Coleta
          </label>
        </div>

        <button className="rel-btn-limpar" onClick={() => {
          setBusca("");
                ...
                return (
                  <tr key={reg.id}>
                    ...
                    <td className="font-mono">
                      {reg.login ? (reg.login.includes(" - ") ? reg.login.split(" - ")[1] : reg.login) : "-"}
                    </td>
                    <td>{reg.olt}</td>
                    ...
                    <td>
                      <div className="rel-status-container">
                        <select
                          ...
                          className={`rel-select-status ${
                            reg.status === "Feito"
                              ? "feito"
                              : reg.status === "Em Andamento"
                              ? "em-andamento"
                              : reg.status === "Ausente (Retorno)"
                              ? "ausente"
                              : "nao-verificado"
                          }`}
                        >
